...
This is a comprehensive, WiFi-enabled, touch-screen smart ticker for stocks and weather. It's built for the "Cheap Yellow Display" (CYD) ESP32 board and is controlled by a modern web interface, including Over-the-Air (OTA) update capabilities. In my case, this has been built for the TENSTAR ESP32 variant, readily found on the likes of AliExpress. (ILI9342 / ILI9341 with TOUCH XPT2046)

The device displays real-time stock quotes or weather forecasts and automatically rotates through user-defined lists for each. The screen can be tapped at any time to toggle between modes.

![1763308303999](image/README/1763308303999.png)

![1763312809400](image/README/1763312809400.jpg)

## Features

* **Dual Mode:** Displays real-time stock quotes or current weather.
...
* **Full Web Control Panel:** A multi-tabbed web interface for full control:
  ...
  * **Rotation Lists:** Add or remove items from the Stock and Weather rotation lists. The device cycles through these lists automatically.
  * **Network Config:** Change the WiFi network the device is connected to.
  * **OTA Updates:** Upload new `firmware.bin` files directly from your browser.
* **Smart APIs:**
  * **Stocks:** Uses the Finnhub API for real-time quotes.
  * **Weather:** Uses the Open-Meteo API, including the Geocoding API to find any location by name.
* **Responsive GUI:** The web interface is responsive for both desktop and mobile, with a themed, non-blocking UI.
...
2. **Web GUI Control:** When you add a new stock in the web GUI, your browser sends an API call (e.g., `/add_stock?ticker=TSLA`) back to the ESP32. The server code in `web_server.cpp` receives this, updates the list in its memory, and sends an "OK" response to your browser.
3. **ESP32 as a Client:** The device's main loop in `main.cpp` is responsible for displaying data. When it's time to fetch an update (e.g., for "NVDA"), the ESP32 acts as a client. It sends its *own* HTTP request out to the internet to the  **Finnhub API** , gets the stock price, and then draws it on the screen.
4. **Weather Geocoding:** When fetching weather for "London", the device *first* sends a request to the **Open-Meteo Geocoding API** to get the latitude and longitude. Once it has those, it sends a *second* request to the **Open-Meteo Forecast API** to get the current weather for those coordinates.
...
### Step 5: Add Your Finnhub API Key

The stock ticker requires a free API key from [finnhub.io](https://finnhub.io "null").

1. Go to [finnhub.io](https://finnhub.io "null") and sign up for a free account.
2. On your dashboard, you will see your  **API Key** . Copy it.
3. In VS Code, open `src/stocks.cpp`.
4. On or around  **line 21** , find this line:
   ```
   String apiurl = "[https://finnhub.io/api/v1/quote?symbol=](https://finnhub.io/api/v1/quote?symbol=)" + ticker + "&token=c76v6u2ad3iaenbsl5dg";
   ```
5. Replace `c76v6u2ad3iaenbsl5dg` with  **your new API key** .

### Step 6: Add Your WiFi Credentials

1. In VS Code, open the project folder.
2. Navigate to `src/config.cpp`.
3. On lines 9 and 10, change the `ssid` and `password` variables to match your home WiFi network.
   ```
   // WIFI & WEB
...
* **Touch:** Tap the screen *at any time* to toggle between the Stock page and the Weather page.
* **Rotation:** Every 1 minute, the device will automatically load the next item in the active list (e.g., it will cycle through all stocks, and if you switch to weather, it will cycle through all locations).

### The Web GUI

Open a browser on your phone or computer and go to: **`http://esp32-ticker.local`**

The web interface has four tabs:

* **One-Off Fetch:** Lets you immediately fetch any stock or weather location. This also resets the 1-minute rotation timer.
* **Rotation:** This is the main settings page.
  * See the lists of all stocks and locations in the rotation.
  * Add new items by typing in the box and clicking "Add".
  * Remove items by clicking the **`×`** icon.
  * The UI is non-blocking; you can add/remove items without the page reloading.
* **Network:**
  * Shows the WiFi network you are currently connected to.
  * Lets you enter a new SSID and Password to connect to a different network. The device will reboot and attempt to connect.
* **Update:**
